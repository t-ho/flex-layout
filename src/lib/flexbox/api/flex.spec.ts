    ...
    it('should fallback the default layout properly', () => {
      fixture = componentWithTemplate(`
        <div fxLayout="column">
          ...
          <div fxFlex="auto" fxFlex.gt-sm="23.4">  </div>
          <div fxFlex="auto" fxFlex.gt-sm="23.4">  </div>
        </div>
      `);
      ...
      expect(nodes[0].nativeElement).toHaveCssStyle({'flex': '1 1 50%'});
      expect(nodes[1].nativeElement).toHaveCssStyle({'flex': '1 1 23.4%'});
      expect(nodes[2].nativeElement).toHaveCssStyle({'flex': '1 1 23.4%'});
    });
  });
});
